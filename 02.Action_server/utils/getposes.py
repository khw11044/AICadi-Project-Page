...
from lib.hrnet.gen_kpts import gen_frames_kpts as hrnet_pose
from lib.hrnet.gen_kpts import gen_frames_yolo as yolo_pose
import numpy as np 
import argparse
import torch
...
def get_pose2D(frames_path):

    print('\nGenerating 2D pose...')
    keypoints, scores, width, height, frames = hrnet_pose(frames_path, det_dim=416, num_peroson=1, gen_output=True)
    ...
    return kps_conf[0], width, height, frames


def get_pose2D_yolov11(frames_path):

    print('\nGenerating 2D pose...')
    keypoints, scores, width, height, frames = yolo_pose(frames_path)
    scores = scores.reshape(scores.shape[0],scores.shape[1],scores.shape[2],1)
    kps_conf = np.concatenate([keypoints, scores], axis=3)
    # re_kpts = revise_kpts(keypoints, scores, valid_frames)
    print('Generating 2D pose successful!')
...
def get_pose3D(poses, width, height, model_name, frames, output_dir):
    ...
    args.previous_dir = './models/poseformer/' # 'checkpoint/'
    ...
    ## input
    kps_conf = poses.reshape(1, -1, 17, 3)
    ...
    for i in tqdm(range(len(image_2d_dir))):
        ...
        ## save
        output_dir_pose = output_dir + result_dir
    ...
    shutil.rmtree(os.path.join(output_dir +'/pose2D/'))
    shutil.rmtree(output_dir_3D)
