...
for action in actions:
    no_sequences = 60 if action == "emergency" else 100
...
def train_and_validate(model, train_loader, val_loader, criterion, optimizer, num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            torch.save(model.state_dict(), './models/action/best_kpcl.pth')
...
def evaluate_model(model, test_loader):
    model.load_state_dict(torch.load('./models/action/best_kpcl.pth'))
    model.eval()
    correct = 0
    with torch.no_grad():
...
# 테스트 데이터셋 로드 및 평가
test_loader = DataLoader(val_dataset, batch_size=128, shuffle=False)  # 예시로 Validation 데이터 재사용
