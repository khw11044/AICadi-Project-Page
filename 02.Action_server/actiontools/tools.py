...
def drawkeypoints(keypoints, frame, color=(0, 255, 0), thickness=5):
    for keypoint in keypoints:
        # 신뢰도가 50% 이상인 포인트만 처리
        points = [(int(x), int(y)) if conf > 0.2 else None for x, y, conf in keypoint]
...
def show2Dpose_h36m(keypoints, frame):
    # color = (0, 0, 0)
...
def prob_viz(res, actions, input_frame):
    colors = [(245,117,16), (117,245,16), (16,117,245), (55,55,255)]
    ...
    for num, prob in enumerate(res):
        ...
        cv2.putText(output_frame, f"{int(prob*100)}", (150, 85+num*40), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,0), 1, cv2.LINE_AA)
